    ...
    return (
        <div className='page-nav-bar'>
            ...
            {(paginateInfo.prevPage !== 1 && paginateInfo.page !== 1) && 
                <>
                    <button type='button' className='btn-link' onClick={() => navigate(`/blog_author?page=1&sortOrd=${sortOrd}&limit=${limitVal}`)}>1</button>
                    &nbsp;/&nbsp;
                </>
            }
            {paginateInfo.hasPrevPage &&
                <>
                    <button type='button' className='btn-link' onClick={() => navigate(`/blog_author?page=${paginateInfo.prevPage}&sortOrd=${sortOrd}&limit=${limitVal}`)}>{paginateInfo.prevPage}</button>
                    &nbsp;&lt;&nbsp;
                </>
            }
            ...
            {paginateInfo.hasNextPage &&
                <>
                    &nbsp;&gt;&nbsp;
                    <button type='button' className='btn-link' onClick={() => navigate(`/blog_author?page=${paginateInfo.nextPage}&sortOrd=${sortOrd}&limit=${limitVal}`)}>{paginateInfo.nextPage}</button>
                </>
            }
            {(paginateInfo.nextPage !== paginateInfo.totalPages && paginateInfo.page !== paginateInfo.totalPages) &&
                <>
                    &nbsp;/&nbsp;
                    <button type='button' className='btn-link' onClick={() => navigate(`/blog_author?page=${paginateInfo.totalPages}&sortOrd=${sortOrd}&limit=${limitVal}`)}>{paginateInfo.totalPages}</button>
                </>
            }
        </div>
    );
