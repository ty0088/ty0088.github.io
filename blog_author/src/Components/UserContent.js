...
const UserContent = ({ currUser, userId, userType, userPosts, userComments, setScrollComId }) => {
    ...
    const commentPostClick = (comment) => {
        if (comment.post.user === currUser.user_id) {
            ...
            window.open(process.env.NODE_ENV === 'production' ? `https://ty0088.github.io/blog_reader#/blog_reader/post/${comment.post._id}` : `${process.env.REACT_APP_BLOG_READER_URL}/post/${comment.post._id}`);
        }
                        ...
                        userPosts.map((post, i) => {
                            return (
                                <li key={i}>
                                    ...
                                    {currUser.user_id !== userId &&
                                        <button type='button' className='button-link' onClick={() => window.open(process.env.NODE_ENV === 'production' ? `https://ty0088.github.io/blog_reader#/blog_reader/post/${post._id}` : `${process.env.REACT_APP_BLOG_READER_URL}/post/${post._id}`)}>{post.title}</button>
                                    }
