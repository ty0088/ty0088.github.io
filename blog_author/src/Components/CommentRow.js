...
const CommentRow = ({ postId, fetchData, currUser, comment, setEditCommentId, scrollNewComFlag, setNewComFlag }) => {
    ...
    return (
        <div className='comment-row' ref={ref} data-commentid={comment._id}>
            ...
            <div className='comment-info'>
                {comment.user ? <Link to={`/blog_author/user/${comment.user._id}`}>{comment.user.display_name} [{comment.user.user_type}]</Link> : '(User not found) '}
