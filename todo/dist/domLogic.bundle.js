...
const domController = (() => {
    ...
    function clickController(event) {
        ...
        if (clickType === 'all') {
            refreshContent(_filterLogic__WEBPACK_IMPORTED_MODULE_1__.filterLists.byAll());
            showHeader('All');
        } else if (clickType === 'today') {
            const list = _filterLogic__WEBPACK_IMPORTED_MODULE_1__.filterLists.byToday();
            const sortedList = sortByUnchecked(list);
            refreshContent(sortedList);
            showHeader('Today');
        } else if (clickType === 'week') {
            const list = _filterLogic__WEBPACK_IMPORTED_MODULE_1__.filterLists.byWeek();
            const sortedList = sortByUnchecked(list);
            refreshContent(sortedList);
            showHeader('Week');
        } else if (clickType === 'comp') {
            refreshContent(_filterLogic__WEBPACK_IMPORTED_MODULE_1__.filterLists.byCompleted());
            showHeader('Completed');
        } else if (clickType === 'proj') {
            const projName = event.target.textContent;
            const list = _filterLogic__WEBPACK_IMPORTED_MODULE_1__.filterLists.byProject(projName);
            const sortedList = sortByUnchecked(list);
            refreshContent(sortedList);
            showHeader(projName);
...
domController.refreshContent(_filterLogic__WEBPACK_IMPORTED_MODULE_1__.filterLists.byAll());
